...
@task
def install(c):
    dist_path = os.path.join(DIST_DIR, "crackwatch_" + MANIFEST['guid'])
    build(c, output=dist_path)


@task
def pack(c):
    output = "crackwatch_" + MANIFEST['guid']
    build(c, output=output, ziparchive='crackwatch_v{}.zip'.format(MANIFEST['version']))
    rmtree(output)